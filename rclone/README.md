...
| Ruta | Descripción |
|------|-------------|
| `~/Config/rclone/setup-rclone.sh` | Instalación desde cero y configuración completa |
| `~/Config/rclone/backup-rclone-weekly.sh` | Script de backup semanal de `rclone.conf` |
| `~/Config/rclone/rclone.conf` | Copia estática actualizada del config |
| `~/Config/rclone/rclone.conf.*` | Backups con fecha |
...
### Instalación inicial
```bash
bash ~/Config/rclone/setup-rclone.sh
```
El script:
...
### Backup manual
```bash
~/Config/rclone/backup-rclone-weekly.sh
```
...
Las credenciales están guardadas en `~/Config/rclone/credenciales.txt` con permisos 600. No incluir en el README para evitar exposición.

## 🗓️ Backup automático

* Frecuencia: cada domingo 03:00
* Retención: 30 días
* Ubicación: `~/Config/rclone/`

## 🛠️ Comandos rápidos

| Acción | Comando |
|--------|---------|
| Instalar / reconfigurar todo | `bash ~/Config/rclone/setup-rclone.sh` |
...
| Backup manual | `~/Config/rclone/backup-rclone-weekly.sh` |
...
| Restaurar config desde backup | `cp ~/Config/rclone/rclone.conf ~/.config/rclone/rclone.conf && systemctl --user restart rclone-gdrive.service` |
