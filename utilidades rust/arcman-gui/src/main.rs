...
async fn clean_aur_cache() -> Result<String, String> {
    let pm = detect_pm();
    match pm {
        "yay" => run_priv("yay".to_string(), vec!["-Sc".to_string(), "--noconfirm".to_string(), "--color=never".to_string()]).await,
        "paru" => run_priv("paru".to_string(), vec!["-Sc".to_string(), "--noconfirm".to_string(), "--color=never".to_string()]).await,
        _ => Ok("No hay yay/paru para limpiar caché AUR.".to_string()),
    }
}
    ...
    fn update(&mut self, msg: Message) -> Command<Message> {
        match msg {
            ...
            Message::RunSearch => {
                ...
                if q.is_empty() {
                    self.error = Some("Ingresa un término de búsqueda.".to_string());
                    return Command::none();
                }
                self.loading = true;
                self.output.clear();
            ...
            Message::RunInstall => {
                ...
                if pkgs.is_empty() {
                    self.error = Some("Ingresa uno o más paquetes.".to_string());
                    return Command::none();
                }
                self.confirmation = Some(Confirmation {
                    ...
                    async {
                        ...
                        match result {
                            Ok(Ok(output)) => {
                                ...
                                if s.trim().is_empty() {
                                    Ok("No hay paquetes huérfanos.".to_string())
                                } else {
                                    Ok(s.trim().to_string())
                                }
                            }
                            _ => Ok("No hay paquetes huérfanos.".to_string()),
                        }
                    },
                    Message::TaskResult,
                )
            ...
            Message::RunOrphansClean => {
                ...
                if orphans.is_empty() {
                    self.error = Some("No hay huérfanos para eliminar.".to_string());
                    return Command::none();
                }
                self.confirmation = Some(Confirmation {
                    action: PendingAction::OrphansClean(orphans),
                    description: format!("Eliminar paquetes huérfanos"),
                });
                Command::none()
            }
            Message::RunClean => {
                self.confirmation = Some(Confirmation {
                    ...
                    description: "Limpiar caché de pacman".to_string(),
                });
                Command::none()
            }
            Message::RunFull => {
                self.confirmation = Some(Confirmation {
                    ...
                    description: "Mantenimiento completo: actualizar + limpiar caché + eliminar huérfanos".to_string(),
                });
                Command::none()
            }
            ...
            Message::TaskResult(result) => {
                ...
                if self.screen == Screen::Orphans {
                    self.orphans_found = self
                        ...
                        .lines()
                        .filter(|l| {
                            !l.is_empty() && l != &"No hay paquetes huérfanos."
                        })
                        .map(String::from)
                        .collect();
                }
...
impl App {
    ...
    fn render_status(&self) -> Element<'_, Message> {
        ...
        let info = text(
            "Muestra información del sistema: gestores de paquetes, \
    ...
    fn render_update(&self) -> Element<'_, Message> {
        ...
        if pm == "none" {
            body = body.push(
                container(
                    text("No se encontró ningún gestor de paquetes (pacman).")
        ...
        self.page_content("Actualizar sistema", "Mantiene tu sistema al día.", body.into())
    }
    ...
    fn render_install(&self) -> Element<'_, Message> {
        ...
        let body = column![
            text("Instala uno o más paquetes (separados por espacio).").size(14),
            Space::with_height(10.0),
            input,
            Space::with_height(10.0),
            btn,
    ...
    fn render_remove(&self) -> Element<'_, Message> {
        ...
        let body = column![
            text(
                "Elimina uno o más paquetes (separados por espacio). \
                 Se eliminarán también dependencias innecesarias.",
            )
            .size(14),
            Space::with_height(10.0),
            input,
    ...
    fn render_orphans(&self) -> Element<'_, Message> {
        ...
        if self.orphans_found.is_empty() {
            let info = text(
                "Busca y elimina paquetes huérfanos (dependencias no utilizadas \
                 por ningún programa).",
            )
            .size(14);
            let btn = self.action_button("\u{F0F0}  Buscar huérfanos", Message::RunOrphans);
            body = body.push(info).push(Space::with_height(5.0)).push(btn);
        } else {
            body = body.push(
                text(format!(
                    "{} paquete(s) huérfano(s) encontrados:",
                    self.orphans_found.len()
                ))
                .size(14),
            );
            body = body.push(Space::with_height(5.0));
            let btn = self.action_button("\u{F0F0}  Eliminar huérfanos", Message::RunOrphansClean);
            body = body.push(btn);
        }
    ...
    fn render_clean(&self) -> Element<'_, Message> {
        ...
        let info = text(
            "Limpia la caché de paquetes de pacman, \
             manteniendo solo las últimas 3 versiones.",
        )
        .size(14);
        body = body.push(info).push(Space::with_height(5.0));

        if which("paccache") {
            let btn = self.action_button("\u{F12D}  Limpiar caché", Message::RunClean);
            body = body.push(btn);
        } else {
            body = body.push(
                container(
                    text(
                        "paccache no está instalado. \
                         Instala pacman-contrib para esta función.",
                    )
        ...
        self.page_content("Limpiar caché", "Libera espacio en el disco.", body.into())
    }
    ...
    fn render_full(&self) -> Element<'_, Message> {
        ...
        let info = text(
            "Realiza un mantenimiento completo del sistema: \
             actualización, limpieza de caché y eliminación de paquetes huérfanos.",
        )
        .size(14);
    ...
    fn render_about(&self) -> Element<'_, Message> {
        ...
        let body = column![
            ...
            Space::with_height(10.0),
            text("Versión: 0.2.0 (GUI)").size(14),
            text("Interfaz gráfica construida con Iced (Rust).").size(14),
            Space::with_height(10.0),
        ...
        self.page_content("Acerca de", "Información sobre la aplicación.", body.into())
    }
        ...
        match action {
                ...
                Command::perform(
                    async move {
                        if !check_network().await {
                            return Err("No hay conexión a internet. Cancela la operación.".to_string());
                        }
                ...
                Command::perform(
                    async {
                        ...
                        if which("paccache") {
                            ...
                            out.push_str("paccache no está instalado.\nInstala pacman-contrib.\n");
                        }
                        out.push('\n');
                        out.push_str("[AUR] Limpiando caché de yay/paru...\n");
                        match clean_aur_cache().await {
                            Ok(s) => out.push_str(&s),
                            Err(e) => out.push_str(&e),
                        }
                        Ok(out)
            ...
            PendingAction::Full => Command::perform(
                async {
                    ...
                    if !check_network().await {
                        out.push_str("No hay conexión a internet. Aborta el mantenimiento.\n");
                        out.push('\n');
                        out.push_str(&"=".repeat(50));
                    ...
                    match pacman_out {
                        Ok(Ok(output)) => {
                            ...
                            let list: Vec<&str> =
                                stdout.lines().filter(|l| !l.is_empty()).collect();
                            if list.is_empty() {
                                out.push_str("No hay paquetes huérfanos.\n");
                            } else {
                                out.push_str(&format!("{} huérfanos encontrados.\n", list.len()));
                        let mut args = vec!["-Rns".to_string(), "--noconfirm".to_string(), "--color=never".to_string()];
                        ...
                        _ => out.push_str("No se pudo buscar huérfanos.\n"),
                    }
        ...
        let dialog = container(
            column![
                row![
                    ...
                    horizontal_space(),
                    text("Confirmar operación").size(18),
                    horizontal_space(),
                    text("").size(22),
                ]
                .align_items(Alignment::Center),
...
pub fn main() -> iced::Result {
    // Usar la GPU NVIDIA (dGPU) en lugar de la iGPU AMD integrada.
    // wgpu por defecto elige LowPower (iGPU); con doble GPU eso causa
    // artefactos al cambiar tema y fallos de dmabuf en Wayland.
