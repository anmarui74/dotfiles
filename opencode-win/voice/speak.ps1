...
$script:KokoroTtsPort = if ($env:KOKORO_TTS_PORT) { [int]$env:KOKORO_TTS_PORT } else { 4210 }

function Test-KokoroServer([int]$port) {
  try {
...
function Start-KokoroServer([int]$port) {
  $venvpy = Resolve-VenvPython
  $serverPy = Join-Path $env:USERPROFILE '.config\opencode\voice\kokoro-server.py'
  if (-not $venvpy -or -not (Test-Path -LiteralPath $serverPy)) { return $false }
  try {
    Start-Process -FilePath $venvpy -ArgumentList @("`"$serverPy`"", '--port', "$port") -WindowStyle Hidden | Out-Null
  } catch { return $false }
  # Esperar a que cargue el modelo (hasta ~20 s)
  for ($i = 0; $i -lt 40; $i++) {
    Start-Sleep -Milliseconds 500
    if (Test-KokoroServer $port) { return $true }
  }
  return $false
}

function Speak-KokoroServer([string]$text, [string]$voice, [double]$rate, [int]$port) {
  $body = @{ text = $text; voice = $voice; speed = $rate } | ConvertTo-Json
  $out = Join-Path $TmpDir ("kokoro-srv-{0}.wav" -f ([guid]::NewGuid().ToString('N')))
  $resp = Invoke-WebRequest -Uri "http://127.0.0.1:$port/tts" -Method Post -Body $body `
...
elseif ($Engine -eq 'edge') {
  ...
  # Ruta rapida: servidor Kokoro persistente (modelo ya cargado en GPU)
  $port = $script:KokoroTtsPort
  $file = $null
  if (-not (Test-KokoroServer $port)) { Start-KokoroServer $port | Out-Null }
  if (Test-KokoroServer $port) {
    try { $file = Speak-KokoroServer $final $KokoroVoice $Rate $port } catch { $file = $null }
  }
  # Fallback: proceso efimero (carga el modelo cada vez, ~2 s)
  if (-not $file) { $file = Speak-Kokoro $final $KokoroVoice $Rate }
