...
function vlog(m) { try { fs.appendFileSync(VOICE_LOG, new Date().toISOString() + " " + m + "\n"); } catch {} }

const MODELS_DIRS = [
  "C:\\Users\\evo01\\.local\\models\\whisper",
...
function startRecording(kv, toast, logger) {
  ...
  soxStderr = "";
  const mic = kv.get("stt.mic", "") || "Micrófono (TONOR TD510 Air Mic)";
  logger?.log("STT", `Starting recording mic=${mic}`, "debug");
  ...
  vlog("record start mic=" + mic);
  soxProc = spawn(
    "C:\\Users\\evo01\\.local\\bin\\ffmpeg\\bin\\ffmpeg.exe",
    [
      "-hide_banner",
      "-loglevel",
      "error",
  ...
  soxProc.on("exit", (code) => {
    soxProc = null;
    vlog("ffmpeg exit code=" + code + " stderr=" + soxStderr.trim().slice(0, 200));
    logger?.log(
      "STT",
...
function stopRecording(logger) {
  ...
  if (soxProc) {
    try {
      if (soxProc.stdin && !soxProc.stdin.destroyed) {
...
function checkAudioSilence(wavPath) {
  try {
    ...
    const headerSize = 44;
    // Crear copia del buffer para evitar problemas de alineación en Windows
    const audioData = buf.slice(headerSize);
    ...
    return false;  // En caso de error, NO asumir silencio (dejar que whisper decida)
  }
}
...
function transcribe(kv, logger) {
  ...
  return new Promise((resolve) => {
    ...
    let stderr = "";
    const proc = spawn(
      "C:\\Users\\evo01\\.local\\bin\\whisper.cpp-cuda\\Release\\whisper-cli.exe",
      ["-m", mp, "-f", WAV_FILE, "-np", "-nt", "-l", "es"],
      {
...
async function doTranscribePipeline(
  ...
  try {
    ...
    try { vlog("pipeline wav=" + (fs.existsSync(WAV_FILE) ? fs.statSync(WAV_FILE).size : "missing")); } catch {}
