...
.DESCRIPTION
  Deja OpenCode totalmente operativo instalando y configurando todo lo necesario:
    ...
    5. Perfil de PowerShell con los lanzadores ocv / ocv-local / ocv-cloud / ocv-status / tts-server
...
$textFiles = @(
  "opencode.jsonc", "opencode-local.json", "opencode-cloud.json", "tui.json", "cli.json", "AGENTS.md",
...
# Carpetas completas (voz + plugins TUI/CLI): scripts de voz, plugin de voz y
# plugin de barra lateral MiMo (se auto-descubre desde plugins\<nombre>\tui.tsx)
foreach ($rel in @("voice", "opencode-voice-modified", "plugins")) {
  $s = Join-Path $ConfigSrc $rel
  if (Test-Path -LiteralPath $s) {
...
$checks["Config global"] = (Test-Path -LiteralPath (Join-Path $TargetConfig "opencode.jsonc"))
$checks["Config local"] = (Test-Path -LiteralPath (Join-Path $TargetConfig "opencode-local.json"))
$checks["Config cloud"] = (Test-Path -LiteralPath (Join-Path $TargetConfig "opencode-cloud.json"))
$checks["cli.json (V2)"] = (Test-Path -LiteralPath (Join-Path $TargetConfig "cli.json"))
$checks["Plugin sidebar"] = (Test-Path -LiteralPath (Join-Path $TargetConfig "plugins\antonio.sidebar-mimo\tui.tsx"))
$checks["Plugin voz"]   = (Test-Path -LiteralPath (Join-Path $TargetConfig "opencode-voice-modified\tui.js"))
$checks["Servidor TTS"] = (Test-Path -LiteralPath (Join-Path $TargetConfig "voice\kokoro-server.py"))
