...
foreach ($f in @("opencode.jsonc", "opencode-local.json", "opencode-cloud.json", "tui.json", "cli.json", "AGENTS.md", "README.md", "lmstudio-proxy.py", "start-lmstudio.ps1")) {
  $p = Join-Path $BackupRoot "config\$f"
  if (Test-Path -LiteralPath $p) { OK "config: $f" } else { ERR2 "Falta en config: $f" }
}

# Plugin de voz (con entrada V2 tui.js) y stack de voz (scripts)
foreach ($f in @(
    "opencode-voice-modified\index.js",
    "opencode-voice-modified\tui.js",
    "opencode-voice-modified\package.json",
    "voice\speak.ps1",
    "voice\kokoro-tts.py",
  ...
  $p = Join-Path $BackupRoot "config\$f"
  if (Test-Path -LiteralPath $p) { OK "voz: $f" } else { ERR2 "Falta en config: $f" }
}

# Plugin TUI de la barra lateral (auto-descubierto por V2)
$sidebar = Join-Path $BackupRoot "config\plugins\antonio.sidebar-mimo\tui.tsx"
if (Test-Path -LiteralPath $sidebar) { OK "plugin sidebar: antonio.sidebar-mimo" }
else { ERR2 "Falta el plugin de barra lateral en config\plugins\antonio.sidebar-mimo\tui.tsx" }
