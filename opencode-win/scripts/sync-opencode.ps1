...
Write-Output ''
Write-Output '🎛️  Plugins TUI/CLI (entrypoints .tsx/.js/.ts):'
foreach ($d in @('plugins')) {
  $s = Join-Path $ORIGEN $d
  if (Test-Path $s) {
    Get-ChildItem -LiteralPath $s -File -Recurse | ForEach-Object {
...
foreach ($d in @('voice', 'opencode-voice-modified')) {
  $s = Join-Path $ORIGEN $d
  if (Test-Path $s) {
    Get-ChildItem -LiteralPath $s -File -Recurse | Where-Object { $_.FullName -notmatch '\\venv\\|\\__pycache__\\' } | ForEach-Object {
      $rel = $_.FullName.Substring($s.Length).TrimStart('\', '/')
      Copy-Fichero $_.FullName (Join-Path (Join-Path $DESTINO $d) $rel)
    }
  } else {
...
Write-Output ''
Write-Output '🛠️  Scripts .ps1 (activo → backup):'
$scriptsSrc = Join-Path $ORIGEN 'scripts'
$scriptsDst = 'D:\Linux\Config\opencode-win\scripts'
if (Test-Path $scriptsSrc) {
  New-Item -ItemType Directory -Path $scriptsDst -Force | Out-Null
  Get-ChildItem -LiteralPath $scriptsSrc -Filter '*.ps1' -File | ForEach-Object {
    Copy-Fichero $_.FullName (Join-Path $scriptsDst $_.Name)
  }
} else {
  Write-Output '   ⏭️  Omitido (no existe): scripts'
}
