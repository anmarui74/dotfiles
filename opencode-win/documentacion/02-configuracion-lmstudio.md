...
> ⚠️ **OpenCode V2 2.0.16**: configuración CLI via cli.json, \tui.json obsoleto.
...
> (descarga/carga vía `lms`). En Windows el proxy **no** hace eso: la
> carga del modelo la gestiona `start-lmstudio.ps1`, que es más
> robusto.
...
| Comando | Qué hace |
|---------|----------|
| `ocv` | Ejecuta `start-lmstudio.ps1` + servidor TTS y abre OpenCode con `OPENCODE_CONFIG=opencode.jsonc` (global/base) |
| `ocv-local` | Igual, pero con `opencode-local.json` (agente `local`, solo MCPs `fetch`, `filesystem`, `memory`) |
| `ocv-cloud` | Abre OpenCode con `opencode-cloud.json` (agente `cloud`, providers locales bloqueados, sin LM Studio) |
| `ocv-status` | Muestra estado del servidor (1234), modelo en VRAM, proxy (4001) y servidor TTS (4210) |
