# 🦙 Configuración de Ollama + Proxy (Windows)
Referencia histórica del antiguo proveedor Ollama + LiteLLM en Windows (EN DESUSO)

| ⚙️ Estado | 📅 Fecha | 👤 Usuario |
|-----------|----------|------------|
| ⚠️ Descontinuado (EN DESUSO) | 10/09/2026 · rev. 10/09/2026 | Antonio |

> Referencia histórica del antiguo proveedor Ollama + LiteLLM. En Windows se mantiene **EN DESUSO** igual que en Linux; el provider activo es **LM Studio**.
...
> ⚠️ **EN DESUSO también en Windows.** A diferencia de otros componentes, Ollama **sí tiene versión nativa para Windows** y podría instalarse con:
>
> ```powershell
> winget install Ollama.Ollama
> ```
>
> Pero en este setup Windows **no se usa**: el proveedor local es **LM Studio** (puerto `1234` + proxy `4001`). Se documenta aquí solo como referencia histórica y por si en el futuro se quisiera recuperar.
...
### Archivo: `C:\Users\evo01\.config\opencode\ollama-proxy.py` (archivado/eliminado)

> ⚠️ Este archivo **ya no existe** en la copia de respaldo `C:\Users\evo01\Config\opencode\` (se eliminó en la limpieza del 09/08/2026). Se documenta aquí solo como referencia histórica del bug #34892. En Windows tampoco se ha portado.
...
Del archivo `.env` (`C:\Users\evo01\.config\opencode\.env`):
...
### Archivo: `C:\Users\evo01\.config\opencode\litellm-config.yaml`
...
> ⚠️ **No aplica en Windows (EN DESUSO):** LiteLLM se instalaría vía `pip` (`python -m pip install litellm`), pero al no usarse Ollama tampoco se usa esta capa. Se mantiene como referencia.
...
> En Windows `nvidia-smi` está disponible si hay GPU NVIDIA (suele estar en `C:\Windows\System32\nvidia-smi.exe`). Aun así, esta lógica **no se ha portado**.
...
python C:\Users\evo01\Config\opencode\ollama-proxy.py 4000

# Probar el proxy (PowerShell)
Invoke-RestMethod http://localhost:4000/v1/models

# Probar el proxy (con curl.exe, incluido en Windows 10/11)
...
> ⚠️ **No aplica en Windows:** el proxy `ollama-proxy.py` no se ha portado y Ollama está EN DESUSO. Los comandos anteriores solo serían válidos si se instalara Ollama con `winget install Ollama.Ollama` y se recuperara el proxy.

---

> **Nota:** Actualmente el provider activo en Windows es **LM Studio** (puerto 1234 + proxy 4001). La configuración de Ollama se mantiene como respaldo histórico.

> 📁 `C:\Users\evo01\.config\opencode\` · `C:\Users\evo01\.lmstudio\`
