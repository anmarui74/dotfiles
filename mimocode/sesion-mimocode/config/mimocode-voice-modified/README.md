...
> ⚠️ **Nota sobre este fork (rev. 11/09/2026):** esta copia local está modificada. Se eliminó la
> **normalización por LLM** y la **transcripción vía API** (`sttEndpoint` / `sttModel` / `sttApiKeyEnv`),
> junto con las opciones de prompt personalizado (`sttPrompt`, `ttsAutoPrompt`, `ttsManualPrompt`) y las
> claves `endpoint` / `model` del plugin. Ahora el STT transcribe en local con **whisper-cpp** y el TTS
> limpia el markdown con **regex locales** y locuta con `~/.local/bin/speak-kokoro-gpu`.
> Las secciones de este README que describen el LLM, la API de transcripción o Piper **ya no aplican**
> al código de este directorio. Referencia del original:
> <https://www.npmjs.com/package/@renjfk/opencode-voice>.
...
> [!NOTE]
> **Clobbering default keybinds.** This plugin uses `g` for voice
> recording, but OpenCode assigns `ctrl+r` to session rename by default. Session
> rename is not used frequently and is still accessible via `/rename`, so we
> avoid clobbering the factory default by using `f6`. See
...
| ------------- | ------------ | -------------------------------------- |
| `/stt-record` | `ctrl+alt+r` | Start/stop recording + transcribe      |
