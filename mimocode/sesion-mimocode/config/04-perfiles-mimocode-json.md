...
Perfil por defecto para `mimo` / `mimo-voz`. Incluye el grupo `lite` (`lmstudio/models-qwen3.8-9b`), proveedores `opencode-go`, `nvidia` y `lmstudio`. **El plugin de voz NO va aquí** (los plugins TUI van en `tui.json`). Los campos `voice.asr_model` y `voice.control_model` son valores por defecto del esquema, no es necesario declararlos.
...
> El grupo `lite` (tareas baratas: títulos, resúmenes) sustituye al antiguo `small_model`, marcado como legacy en la wiki de MiMoCode. En el perfil cloud, `lite` apunta a `opencode-go/deepseek-v4-flash`.
