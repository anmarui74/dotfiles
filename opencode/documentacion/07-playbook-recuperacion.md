...
| Grafo de memoria (copia estable → dotfiles) | `~/Config/opencode/data/memory/memory.jsonl` |
| Grafo de memoria (histórico local, NO va a dotfiles) | `~/Config/opencode/backups/mcp-memory-backup-*.jsonl` |
...
### 🔐 Claves y permisos (rev. 22/09/2026)

| Regla | Detalle |
|-------|---------|
| **Las claves SÍ van en `~/Config/opencode/`** | Dentro de los tarballs (`credenciales/auth.json`). También el **instalador las embebe** (`.env` saneado solo en dotfiles) |
| **Las claves NUNCA van en `~/Documentos/dotfiles/`** | Esa es la frontera real |
| **Permisos `600`/`700`** | El tarball se crea con `umask 077` + `chmod 600` y su directorio con `chmod 700` (contiene `auth.json` en claro). Antes quedaban en `644`/`755` |
| **Dotfiles con `--delete-excluded`** | Con solo `--delete`, lo que está en la lista de exclusión **no se purga** del destino |
| **Grafo de memoria** | **No se excluye** de dotfiles, pero va en **UNA sola copia** (`data/memory/memory.jsonl`). Las 179 copias fechadas de `backups/` se dejaron de versionar |
| **`backups/` excluido** | Se excluye **todo** el directorio `backups/` del volcado a dotfiles (desde el 22/09/2026), no solo `backups/opencode/` |
| **Saneado del instalador** | En dotfiles se sustituyen las claves por `TU_CLAVE_AQUI`. **Nunca** sanear el canónico de `sesion-opencode/` |
...
# Herramientas de lint/LSP (V2 no ejecuta LSP; sirven por terminal)
