...
OpenCode **descubre `AGENTS.md` automáticamente** en la raíz de su configuración
(`~/.config/opencode/AGENTS.md`). La clave `instructions` se **retiró** de `opencode.json`
el 19/09/2026: V2 la acepta pero **no carga sus entradas** (era inerte).
...
El archivo se organiza en **5 bloques** (el contenido ha crecido con la migración a V2):

```
1. REGLAS OBLIGATORIAS (APLICAR SIEMPRE)
   ├── Usuario · Idioma · Formato
   ├── Formato de manuales y README (plantilla obligatoria)
   ├── Consultar el tiempo (AEMET OpenData; respaldo wttr.in)
   ├── Consultar hardware (leer data/hardware/index.json)
   ├── Uso de herramientas (actuar, no narrar)
   ├── Shell del sistema (ZSH usuario vs Bash agentes)
   ├── Pronunciación de Antonio (entrada por voz)
   ├── Elevación de privilegios (agente → pkexec; terminal → sudo)
   ├── Consultar documentación oficial ante problemas
   └── Verificar hechos antes de afirmar

2. PROCEDIMIENTOS TÉCNICOS
   ├── OpenCode V2 (2.0.x): cambios importantes (config, plugins TUI, voz, sintaxis, LSP…)
   ├── Sincronización con Config/opencode (estructura ordenada)
   └── Atención a setup-opencode-completo.sh

3. PERSISTENCIA DE DATOS Y RECUPERACIÓN
   ├── Variables de entorno · Inicialización
   ├── Backup y recuperación del grafo de memoria
   ├── Recordatorio MCP memory (entityName)
   ├── Timeline completo (timeline-completo) + vigilancia del PR #26861
   ├── Vigilancia del issue #39164 (check-fix.sh)
   ├── Tecla `l` tragada en la TUI V2 (resuelto)
   └── Directorios de datos

4. AVANZADO (uso con DeepSeek)
   ├── PWAs (abrir/cerrar) · Chrome debug
   ├── Carga automática al abrir opencode/ocv · Perfiles por lanzador
   ├── Proveedor NVIDIA: ranking y metodología de selección
   ├── Iniciar LM Studio manualmente · Liberar VRAM
   └── Tokens/s (TUI, proxy 4001, dashboard 4200)

5. CHECKLIST ANTES DE RESPONDER
```

> 📌 Las referencias a números de línea de las secciones siguientes son **orientativas**:
> el archivo ha crecido con las actualizaciones y la numeración cambia con frecuencia.
...
```
Para preguntas sobre el tiempo, usar la API oficial de AEMET OpenData (curl):
1. GET https://opendata.aemet.es/opendata/api/prediccion/especifica/municipio/horaria/04074?api_key=$AEMET_API_KEY
2. Descargar la URL que devuelve el campo "datos" (válida ~5 min)
ID de Pechina: 04074 · api_key en $AEMET_API_KEY (.env)

⚠️ Respaldo si AEMET falla: https://wttr.in/{ciudad}?format=j1&m&lang=es
```

Razón: AEMET es la fuente oficial española. El `api_key` se carga del `.env`
(`set -a; source ~/.config/opencode/.env; set +a`). La respuesta de AEMET viene en
...
```
- El AGENTE NUNCA usa `sudo` para comandos que requieran contraseña: usa SIEMPRE `pkexec`
- Ejemplo: `pkexec apt update` en vez de `sudo apt update`
- Diferencia clave: `pkexec` es para el agente (no tiene terminal interactiva);
  los scripts que Antonio ejecuta en SU terminal SÍ pueden usar `sudo`
```

Razón: `sudo` pide la contraseña por terminal, pero el agente se ejecuta en un entorno sin terminal interactiva. `pkexec` abre una **ventana gráfica** donde Antonio puede escribir la contraseña cómodamente.

---

## Sección 2: PROCEDIMIENTOS TÉCNICOS

### OpenCode V2 (2.0.x) — cambios importantes

AGENTS.md documenta la migración de V1 (1.18.x) a **V2 (2.0.x)**:

- Config TUI: `tui.json` → **`cli.json`**; los plugins TUI V1 no funcionan (los nuevos van en `plugins/<nombre>/tui.tsx`).
- Ciclo de agentes primarios: **`Shift+Tab`** (ya no `Tab`); fallback `Ctrl+X → A`.
- Sintaxis canónica V2 (`providers`, `agents`, `permissions`, `mcp.servers`, `system`, `disabled`); `small_model` → `agents.title.model` y `disabled_providers` → `experimental.policies`.
- NVIDIA: `providers.nvidia.whitelist` la aplica el plugin `nvidia-filter` (V2 ignora esa clave).
...
2. **Modelo Qwen3.8-9B Q6_K** con 80K de contexto
...
En cloud: agente local desactivado (`agents.local.disabled`), el agente oculto `title` apunta a la nube (`agents.title.model` = `opencode-go/deepseek-v4.1-flash`) y los providers LM Studio/local están bloqueados con `experimental.policies` (`action: provider.use` → `effect: deny`). El antiguo `switch-mcp-profile.sh` está **ELIMINADO** desde el 18/08/2026.

### Proveedor NVIDIA: ranking y metodología

AGENTS.md incluye la metodología (05/09/2026) para verificar/actualizar el whitelist NVIDIA:
catálogo real vía `GET /v1/models` (el catálogo `models.dev` está desactualizado) → HTTP 200 real
...
#### En la TUI — barra lateral (`plugins/sidebar-mimo/tui.tsx`, V2, 16/09/2026)

La barra lateral (Ctrl+X → B) muestra en el bloque **Context** el nº de tokens, el
% usado, el límite del modelo, los t/s de la última respuesta y el coste; debajo,
**Directorio de trabajo** e **Instrucciones**, y la **versión** al final. Funciona
para TODOS los providers.
(En `cli.json` se desactivan `-opencode.sidebar.context` y `-opencode.sidebar.footer`.)
...
- ¿Decimales con coma?
- Si pregunta por el tiempo: ¿he usado AEMET (o wttr.in de respaldo)?
...
| ¿Tiempo con AEMET? | Es la fuente oficial del clima (wttr.in como respaldo) |
